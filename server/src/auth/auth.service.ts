...
import { Credentials, SingupCredentials } from "./Credentials";
...
export class AuthService {
  ...
  async signup(credentials: SingupCredentials): Promise<UserInfo> {
    ...
    if(!user) {
     ...
     async me(authorization: string= " "): Promise<User> {
      ...
      if (!result) {
        throw new NotFoundException(`No resourse was found for ${username}`);
      }
          return result;
        }
        async checkUser(email: string){
          ...
          if (!userStatus) {
            throw new NotFoundException(`No resourse was found for ${email}`);
          }
          return userStatus;
        }
      }
